...
// TODO: 1
const fetchingUserFromInternet = (isOffline = false) => {
  return new Promise((res, rej) => {
    setTimeout(() => {
      if (isOffline) {
        rej(new NetworkError('Gagal mendapatkan data dari internet'), null);
      }
...
const gettingUserName = async () => {
  let user = null
  try {
    user = await fetchingUserFromInternet()
  } catch (error) {
...
console.log( gettingUserName().then(res=>console.log(res)) );
